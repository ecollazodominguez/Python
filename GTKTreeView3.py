...
class Fiestra(Gtk.Window):
    ...
    def __init__(self):
        ...
        # Creamos un modelo filtrado (TreeModelFilter) a partir do modelo (ListStore o TreeStore)
        modeloFiltrado = modelo.filter_new()

        # Indicamos a funcion que nos hará a filtracion
        self.filtradoOcupacion = False
        modeloFiltrado.set_visible_func(self.ocupacion, 75.0)
        ...
        # Ordenar a columna por cantidade de ocupación si se clicka en "OCUPACION"
        ...
        self.chkFiltro = Gtk.CheckButton(label='Filtro ocupacion')
        self.chkFiltro.connect("toggled",self.on_chkFiltro_toggled, self.ocupacion)
    ...
    def on_vista_changed(self, seleccion):
        ...
        print('estrellas:',modelo[punteiro][4])
    ...
    def ocupacion (self, modelo, punteiro, porcentaxeOcupacion):
        if self.filtradoOcupacion is None or self.filtradoOcupacion is False:
            ...
            return modelo[punteiro][2] > porcentaxeOcupacion


    def on_chkFiltro_toggled (self, control,punteiro):
